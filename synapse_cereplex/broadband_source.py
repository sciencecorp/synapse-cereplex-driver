import asyncio
import time

import numpy as np

from cerebus import cbpy

from synapse.api.node_pb2 import NodeType
from synapse.api.nodes.broadband_source_pb2 import BroadbandSourceConfig
from synapse.api.synapse_pb2 import Peripheral
from synapse.server.nodes import BaseNode
from synapse.server.status import Status, StatusCode
from synapse.utils.ndtp_types import ElectricalBroadbandData

from synapse_cereplex.utils.cerebus import (
    BIT_WIDTHS,
    CHANNEL_COUNT,
    CONFIG_MAP_SAMPLE_RATE,
    PERIPHERALS,
    SAMPLE_RATES,
    CerebusConnection,
    SampleGroup,
)


class BroadbandSource(BaseNode):
    def __init__(self, id):
        super().__init__(id, NodeType.kBroadbandSource)
        self.sample_rate_hz = 0
        self.bit_width = 0
        self.connection = CerebusConnection()

    def get_data(self) -> tuple[Status, list[tuple[int, np.ndarray]], int]:
        try:
            # data is a list of [channel_id, np.array(samples, dtype=int16)] tuples
            res, data, t0 = cbpy.trial_continuous(reset=True)

            # t0 is the nanosecond timestamp at sample 0, but it's represented in too
            # few bits and a pain to deal with, so we are overwriting it here with the
            # current time in microseconds
            t0 = int(time.time() * 1e6)

            if res != 0:
                status = Status(
                    code=StatusCode.kInternalError,
                    message=f"failed to read data: code {res}",
                )
                return status, None, None

            if len(data) < 1:
                status = Status()
                return status, None, None

            return Status(), data, t0

        except Exception as e:
            self.logger.error(f"Exception in cbpy.trial_continuous: {e}")
            status = Status(
                code=StatusCode.kInternalError,
                message=f"Exception in cbpy.trial_continuous: {e}",
            )
            return status, None, None

    async def run(self):
        while self.running:
            try:
                status, data, t0 = await asyncio.to_thread(self.get_data)

                if status.ok() and data:
                    await self.emit_data(self._pack(t0, data))
                elif status.ok():
                    pass
                else:
                    self.logger.warn(
                        f"failed to read data: (code: {status.code()}) {status.message()}"
                    )

                await asyncio.sleep(0.001)

            except Exception as e:
                self.logger.warn(f"failed to read data: {e}")

    def config(self):
        c = super().config()
        if self.__config:
            c.broadband_source.CopyFrom(self.__config)
        return c

    def configure(self, config: BroadbandSourceConfig) -> Status:
        self.logger.info(
            f"Configuring BroadbandSource node with configuration {config}"
        )

        # Check for electrode configuration
        if not config.signal or not config.signal.HasField("electrode"):
            return Status(
                code=StatusCode.kInvalidConfiguration,
                message="must provide electrode configuration",
            )

        econfig = config.signal.electrode

        # Validate peripheral_id
        id = config.peripheral_id
        if not id:
            return Status(
                code=StatusCode.kInvalidConfiguration,
                message="must provide peripheral_id",
            )

        ps = [p for p in PERIPHERALS if p.peripheral_id == id]
        if len(ps) < 1:
            return Status(
                code=StatusCode.kInvalidConfiguration,
                message=f"must provide valid peripheral_id: must be {[p.peripheral_id for p in peripherals]}",
            )
        peripheral = ps[0]

        # Validate sample rate
        self.sample_rate_hz = config.sample_rate_hz
        if self.sample_rate_hz not in SAMPLE_RATES:
            return Status(
                code=StatusCode.kInvalidConfiguration,
                message=f"invalid sample rate: must be one of {SAMPLE_RATES}",
            )

        sample_group = CONFIG_MAP_SAMPLE_RATE[self.sample_rate_hz]

        # Validate bit width
        self.bit_width = config.bit_width
        if self.bit_width not in BIT_WIDTHS:
            return Status(
                code=StatusCode.kInvalidConfiguration,
                message=f"invalid bit width: must be one of {BIT_WIDTHS}",
            )

        # Validate Channels
        ch_map = {}
        for ch in econfig.channels:
            ch_id = ch.id

            if ch_id == 0:
                return Status(
                    code=StatusCode.kInvalidConfiguration,
                    message=f"invalid channel id={ch_id}: cereplex is 1-indexed",
                )
            elif ch_id > CHANNEL_COUNT:
                return Status(
                    code=StatusCode.kInvalidConfiguration,
                    message=f"invalid channel id={ch_id}: must be within [1, {CHANNEL_COUNT}]",
                )
            ch_map[ch_id] = ch

        # Configure channels (on / off, sample group / rate)
        for c in range(1, CHANNEL_COUNT + 1):
            ch_sample_group = SampleGroup.NONE

            if c in ch_map:
                ch_sample_group = sample_group

            status = self._configure_channel(peripheral, c, ch_sample_group)
            if not status.ok():
                return status

        # Configure system (bit depth)
        res, _ = cbpy.trial_config(
            reset=True,
            buffer_parameter={
                "absolute_time": True,
                "continuous_length": 30000,
                "event_length": 0,
                "double": self.bit_width == 64,
            },
            noevent=1,
            nocontinuous=0,
            nocomment=1,
        )

        if res != 0:
            return Status(
                code=StatusCode.kInternalError,
                message=f"failed to configure system: code {res}",
            )

        self.__config = config

        return Status()

    def _configure_channel(
        self, peripheral: Peripheral, ch: int, sample_group: SampleGroup
    ) -> Status:
        res = None
        info = None

        self.logger.debug(f"configuring channel {ch} with group {sample_group}")
        try:
            res, info = cbpy.get_channel_config(ch)

            if res != 0:
                return Status(
                    code=StatusCode.kInternalError,
                    message=f"failed to get config for channel {ch}: code {res}",
                )

            info["smpgroup"] = sample_group.value

        except RuntimeError as e:
            return Status(
                code=StatusCode.kInternalError,
                message=f"failed to get config for channel {ch}: {e}",
            )

        try:
            res = cbpy.set_channel_config(ch, info)

        except RuntimeError as e:
            return Status(
                code=StatusCode.kInternalError,
                message=f"failed to set config for channel {ch}: {e}",
            )

        self.logger.debug(f" - configured channel {ch}")

        return Status()

    def _pack(self, t0, data):
        return ElectricalBroadbandData(
            sample_rate=self.sample_rate_hz,
            t0=t0,
            samples=data,
            bit_width=self.bit_width,
            is_signed=True,
        )
